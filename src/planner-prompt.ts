...
export function buildPlannerPrompt(options: PlannerPromptOptions): string {
  ...
  return [
    ...
    "",
    "Then append exactly one final-line marker for plan-stage product review:",
    "`RALPH_PLAN_REVIEW: {\"status\":\"pass\"|\"fail\",\"reason\":\"...\"}`",
    "- The marker must appear exactly once.",
    "- The marker must be on the final non-empty line.",
    "- Missing/invalid marker is treated as deterministic failure.",
    "",
    "---",
    "",
