...
describe("GitHub queue blocked label reconciliation", () => {
  ...
  test("adds status:blocked for queued issues with open dependencies when autoQueue enabled", async () => {
    ...
    expect(calls).toEqual([
      { repo: "3mdistal/ralph", issueNumber: 1, add: ["ralph:status:blocked"], remove: ["ralph:status:queued"] },
    ]);
  });

  test("removes status:blocked for queued issues once dependencies are clear when autoQueue enabled", async () => {
    const now = new Date("2026-01-11T00:00:00.000Z");
    await writeJson(getRalphConfigJsonPath(), {
      queueBackend: "github",
      repos: [
    ...
    stateMod.recordIssueLabelsSnapshot({
      repo: "3mdistal/ralph",
      issue: "3mdistal/ralph#2",
      labels: ["ralph:status:queued", "ralph:status:blocked"],
      at: now.toISOString(),
    });
    ...
    const driver = queueMod.createGitHubQueueDriver({
      ...
      io: {
        ensureWorkflowLabels: async () => ({ ok: true, created: [], updated: [] }),
        listIssueLabels: async () => ["ralph:status:queued", "ralph:status:blocked"],
        reopenIssue: async () => {},
        addIssueLabel: async () => {},
        addIssueLabels: async () => {},
        removeIssueLabel: async () => ({ removed: true }),
    ...
    expect(calls).toEqual([
      { repo: "3mdistal/ralph", issueNumber: 2, add: ["ralph:status:queued"], remove: ["ralph:status:blocked"] },
    ]);
  });

  test("does not add status:blocked when dependency coverage is unknown", async () => {
    const now = new Date("2026-01-11T00:00:00.000Z");
    await writeJson(getRalphConfigJsonPath(), {
      queueBackend: "github",
      repos: [
