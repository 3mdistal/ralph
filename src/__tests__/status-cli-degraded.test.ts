...
function runRalphStatus(args: string[], env: NodeJS.ProcessEnv): { status: number | null; stdout: string; stderr: string } {
  const child = spawnSync(process.execPath, ["src/index.ts", "status", ...args], {
    cwd: REPO_ROOT,
    env,
    encoding: "utf8",
  });
...
describe("ralph status degraded mode", () => {
  test("status --json succeeds with forward-incompatible durable state", async () => {
    const homeDir = await mkdtemp(join(tmpdir(), "ralph-status-home-"));
    const xdgStateHome = await mkdtemp(join(tmpdir(), "ralph-status-xdg-"));
    ...
    try {
      const result = runRalphStatus(["--json"], buildIsolatedEnv(homeDir, xdgStateHome));
