...
describe("github queue core", () => {
  ...
  test("statusToRalphLabelDelta replaces queued when blocked", () => {
  ...
  test("statusToRalphLabelDelta replaces in-progress when blocked", () => {
  ...
  test("deriveRalphStatus returns null for conflicting status labels", () => {
    const status = deriveRalphStatus(["ralph:status:queued", "ralph:status:blocked"], "OPEN");
    expect(status).toBe(null);
  });

  test("deriveRalphStatus treats status:done as done", () => {
