...
describe("durable-state capability evaluator", () => {
  ...
  test("returns unreadable_forward_incompatible when schema is below minReadableSchema", () => {
    ...
    expect(capability.canReadState).toBeFalse();
    expect(capability.canWriteState).toBeFalse();
    expect(capability.requiresMigration).toBeTrue();
  ...
  test("returns readable_writable at maxWritableSchema boundary", () => {
    ...
    expect(capability.canReadState).toBeTrue();
    expect(capability.canWriteState).toBeTrue();
    expect(capability.requiresMigration).toBeFalse();
