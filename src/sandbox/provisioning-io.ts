...
import type { SandboxManifest, SeededIssueRecord, SeededPullRequestRecord } from "./manifest";
...
async function createRepoFromTemplate(plan: ProvisionPlan, ports: Ports): Promise<RepoInfo> {
  ...
  if (plan.templateRef && plan.templateRef !== response.data.default_branch) {
    try {
      await targetClient.request(`/repos/${plan.repoOwner}/${plan.repoName}`, {
        method: "PATCH",
  ...
  return await requestRepoInfo(targetClient, plan.repoFullName);
}
...
async function copyBranchProtection(params: {
  ...
  try {
    ...
    params.warnings.push(`branch protection fetch failed for ${params.branch}: ${error?.message ?? String(error)}`);
    return;
  }
  ...
  try {
    ...
    params.warnings.push(`branch protection copy failed for ${params.branch}: ${error?.message ?? String(error)}`);
  }
}
...
async function copyRulesets(params: {
  ...
  try {
    ...
    params.warnings.push(`ruleset list failed: ${error?.message ?? String(error)}`);
    return;
  }
  ...
  for (const ruleset of rulesets) {
    ...
    try {
      ...
      params.warnings.push(`ruleset copy failed (${ruleset?.name ?? ruleset?.id ?? "unknown"}): ${error?.message ?? String(error)}`);
    }
  }
}
